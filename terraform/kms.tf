...
resource "aws_route53_zone" "seehmat_new" {
  name = var.domain_name
  tags = {
    Description = "New hosted zone for seehmat.com"
  }
}
    ...
    "Statement": [
      ...
      },
      {
        ...
        "Condition": {
          ...
          "ArnLike": {
            "aws:SourceArn": "arn:aws:route53:::hostedzone/${aws_route53_zone.seehmat_new.id}"
          }
        }
      }
    ]
...
resource "aws_route53_key_signing_key" "ksk" {
  hosted_zone_id             = aws_route53_zone.seehmat_new.id
